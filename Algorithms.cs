...
namespace CpuSchedulingWinForms
{
    public static class Algorithms
    {
        ...
        public static void srtfAlgorithm(string userInput)
        {
            ...
            double[] wtp = new double[np];
            double[] at = new double[np];
            int[] finishTime = new int[np];
            bool[] isCompleted = new bool[np];
            string[] output1 = new string[npX2];
            double twt = 0.0, awt;
            int num, currentTime = 0, completed = 0;
            ...
            if (result == DialogResult.Yes)
            {
                ...
                for (num = 0; num < np; num++)
                {
                    ...
                    at[num] = Convert.ToDouble(arrivalInput);

                    remainingTime[num] = bp[num];
                    isCompleted[num] = false;
                }
                while (completed < np)
                {
                    int shortest = -1;
                    ...
                    for (num = 0; num < np; num++)
                    {
                        if (at[num] <= currentTime && !isCompleted[num] && remainingTime[num] < minRemaining && remainingTime[num] > 0)
                        {
                            shortest = num;
                            minRemaining = remainingTime[num];
                        }
                    }
                    ...
                    if (remainingTime[shortest] == 0)
                    {
                        ...
                        wtp[shortest] = finishTime[shortest] - at[shortest] - bp[shortest];

                        MessageBox.Show(
                            "Process P" + (shortest + 1) + " completed.\n" +
                            "Finish time: " + finishTime[shortest] + "\n" +
                            "Waiting time " + wtp[shortest],
                ...
                //Final results
                string output = "Process\tArrival\tBurst\tFinish\tWaiting\n";
                for (num = 0; num < np; num++)
                {
                    output += $"P{num + 1}\t{at[num]}\t{bp[num]}\t{finishTime[num]}\t{wtp[num]}\n";
                }
        ...
        public static void mlfqAlgorithm(string userInput) //MLFQ MLFQ MLFQMLFQMLFQMLMFLQMFLMQLFMLQMFLMQLMFLQMFLQMLFMQLFMLQMFLQMLFMQLFMQLFMQLMFQLMFLQMFLQMF HERE
        {
            int np = Convert.ToInt16(userInput);
            int npX2 = np * 2;

            double[] foregroundBurst = new double[np]; //High priority (queue 1)
            double[] backgroundBurst = new double[np]; //Low priority (queue 2)
            double[] wtp = new double[np];
            string[] output1 = new string[npX2];
            double twt = 0.0, awt;
            int num;

            DialogResult result = MessageBox.Show("Multilevel Queue Scheduling ", "", MessageBoxButtons.YesNo, MessageBoxIcon.Information);
            ...
            if (result == DialogResult.Yes)
            {
                for (num = 0; num <= np - 1; num++)
                {
                    ...
                    string queueInput =
                    Microsoft.VisualBasic.Interaction.InputBox("Enter Queue (1=Foreground, 2=Background): ",
                                                       "Queue for P" + (num + 1),
                                                       "",
                                                       -1, -1);

                    int queue = Convert.ToInt16(queueInput);
                    double burst = Convert.ToDouble(burstInput);

                    if (queue == 1)
                    {
                        foregroundBurst[num] = burst;
                    }
                    else
                    {
                        backgroundBurst[num] = burst;
                    }
                }
                ...
                for (num = 0; num <= np - 1; num++)
                {
                    if (foregroundBurst[num] > 0)
                    {
                        wtp[num] = currentTime;
                        currentTime += foregroundBurst[num];
                        MessageBox.Show("Foreground Process P" + (num + 1) + " waiting time = " + wtp[num], "Job Queue", MessageBoxButtons.OK, MessageBoxIcon.None);
                    }
                }
                ...
                for (num = 0; num <= np - 1; num++)
                {
                    if (backgroundBurst[num] > 0)
                    {
                        wtp[num] = currentTime;
                        currentTime += backgroundBurst[num];
                        MessageBox.Show("Background Process P" + (num + 1) + " waiting time = " + wtp[num], "Job Queue", MessageBoxButtons.OK, MessageBoxIcon.None);
                    }
                }
