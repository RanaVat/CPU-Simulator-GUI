...
namespace CpuSchedulingWinForms
{
    public partial class CpuScheduler : Form
    {
        ...
        private void btnSRTF_Click(object sender, EventArgs e)
        {
            if (txtProcess.Text != "")
            {
                Algorithms.srtfAlgorithm(txtProcess.Text); // Call SRTF algorithm
                int numberOfProcess = Int16.Parse(txtProcess.Text);

                // Progress bar updates (same logic as FCFS)
                if (numberOfProcess <= 10)
                {
                    this.progressBar1.Increment(4); // CPU progress bar
                    this.progressBar1.SetState(1);  // Green state
                ...
                else if (numberOfProcess > 10)
                {
                    ...
                    this.progressBar2.SetState(3);  // Red state for high load
                }

                // Configure ListView (same columns as FCFS)
                listView1.Clear();
                listView1.View = View.Details;
                listView1.Columns.Add("Process ID", 150, HorizontalAlignment.Center);
                listView1.Columns.Add("Remaining Time", 100, HorizontalAlignment.Center); // Changed from "Quantum Time"

                // Populate ListView with processes
                for (int i = 0; i < numberOfProcess; i++)
                {
                    var item = new ListViewItem();
                    item.Text = "Process " + (i + 1);
                    item.SubItems.Add("Dynamic"); // SRTF updates remaining time dynamically
                    listView1.Items.Add(item);
                }

                // Add summary line
                listView1.Items.Add("\n");
                listView1.Items.Add("CPU handles: " + numberOfProcess + " (SRTF)");
            }
            else
            {
                MessageBox.Show("Enter number of processes", "", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void btnMLFQ_Click(object sender, EventArgs e)
        {
            if (txtProcess.Text != "")
            {
                ...
                // Progress bar updates (same as Round Robin)
                if (numberOfProcess <= 10)
                {
                ...
                // Define quantum times for each queue level (example: 3 levels)
                string[] quantumTimes = { "4", "8", "16" };

                listView1.Clear();
                listView1.View = View.Details;

                // Add columns for Queue Level and Quantum Time
                listView1.Columns.Add("Process ID", 120, HorizontalAlignment.Center);
                listView1.Columns.Add("Queue Level", 120, HorizontalAlignment.Center);
                listView1.Columns.Add("Quantum Time", 100, HorizontalAlignment.Center);
                ...
                for (int i = 0; i < numberOfProcess; i++)
                {
                    ...
                    // Assign random queue level (0, 1, or 2 for 3 queues)
                    int queueLevel = rnd.Next(0, 3);
                    item.SubItems.Add($"Queue {queueLevel + 1}");
                    item.SubItems.Add(quantumTimes[queueLevel]); // Use corresponding QT

                    listView1.Items.Add(item);
                }

                listView1.Items.Add("\n");
                listView1.Items.Add($"CPU handles: {numberOfProcess} (MLFQ)");
            }
            else
            {
                MessageBox.Show("Enter number of processes", "", MessageBoxButtons.OK, MessageBoxIcon.Error);
